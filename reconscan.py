...
def httpEnum(ip_address, port):
    ...
    HTTPSCAN = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt -oA /root/scripts/recon/recon2/results/nmap/%s_http %s" % (port, ip_address, ip_address)
...
def httpsEnum(ip_address, port):
    ...
    print "INFO: Performing nmap web script scan for " + ip_address + ":" + port    
    HTTPSCANS = "nmap -sV -Pn -vv -p %s --script=http-vhosts,http-userdir-enum,http-apache-negotiation,http-backup-finder,http-config-backup,http-default-accounts,http-methods,http-method-tamper,http-passwd,http-robots.txt -oA /root/scripts/recon/recon2/results/nmap/%s_https %s" % (port, ip_address, ip_address)
...
def mssqlEnum(ip_address, port):
    ...
    MSSQLSCAN = "nmap -vv -sV -Pn -p %s --script=ms-sql-info,ms-sql-config,ms-sql-dump-hashes --script-args=mssql.instance-port=1433,smsql.username-sa,mssql.password-sa -oX results/nmap/%s_mssql.xml %s" % (port, ip_address, ip_address)
...
def nmapScan(ip_address):
   ...
   serv_dict = {}
   TCPSCAN = "nmap -vv -Pn -A -sC -sS -T 4 -p- -oA '/root/scripts/recon/recon2/results/nmap/%s' %s"  % (ip_address, ip_address)
   UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 -oA '/root/scripts/recon/recon2/results/nmap/%sU' %s" % (ip_address, ip_address)
